...
function glide_blocks_category( $categories ) {
	$custom_block = array(
		'slug'  => 'glide-blocks',
		'title' => __( 'Glide Blocks', 'databook_td' ),
		'icon'  => 'glide-blocks',
	);
...
if ( function_exists( 'acf_add_options_page' ) ) {
	$option_page = acf_add_options_page(
		array(
			'page_title' => __( 'Theme Options', 'databook_td' ),
			'menu_title' => __( 'Theme Options', 'databook_td' ),
...
function glide_acf_button( $object, $classes = '' ) {
	if ( $object['url'] ) {
		$link = '';
		$img_path = get_template_directory_uri() . '/assets/img/white-arrow.svg';
		$link = "<a href='" . esc_url( $object['url'] ) . "' title='" . esc_html( $object['title'] ) . "' target='" . $object['target'] . "' class='" . $classes . "'><span class='btn-text'>"  . esc_html( $object['title'] ) . '</span><span class="btn-user-arrow"><img src="'.$img_path.'" alt="" width="" height=""></span></a>';
		return $link;
	}
	return null;
}
...
function wysiwyg_render_field_settings( $field ) {
	acf_render_field_setting(
		$field,
		array(
			'label'        => __( 'Height of Editor', 'databook_td' ),
			'instructions' => __( 'Height of Editor after Init', 'databook_td' ),
			'name'         => 'wysiwyg_height',
			'type'         => 'number',
		)
	);
...
/**
 * Allow use of shortcode to ACF wysiwyg and textarea
 */
